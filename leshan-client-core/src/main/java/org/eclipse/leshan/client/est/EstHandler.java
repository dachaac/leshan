...
/**
 * Handle EST session state.
 */
public class EstHandler {

    private boolean sequenceStarted = false;
    private CountDownLatch estLatch = new CountDownLatch(1);

    private final Map<Integer, LwM2mObjectEnabler> objects;

    public EstHandler() {
        objects = new HashMap<Integer, LwM2mObjectEnabler>();
    }
    ...
    public synchronized boolean isSequenceStarted() {
        return sequenceStarted;
    }

    public boolean waitEstFinished(long timeInSeconds) throws InterruptedException {
        return estLatch.await(timeInSeconds, TimeUnit.SECONDS);
    }
