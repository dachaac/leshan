...
public class Security extends BaseInstanceEnabler {
    ...
    private final static List<Integer> supportedResources = Arrays
            .asList(SEC_SERVER_URI, SEC_BOOTSTRAP, SEC_SECURITY_MODE, SEC_PUBKEY_IDENTITY, SEC_SERVER_PUBKEY,
                    SEC_SECRET_KEY, SEC_SERVER_ID, SEC_CERTIFICATE_USAGE);
    ...
    public Security(String serverUri, boolean bootstrapServer, int securityMode, byte[] publicKeyOrIdentity,
            byte[] serverPublicKey, byte[] secretKey, Integer shortServerId, int certificateUsage) {
    ...
    public static Security x509(String serverUri, int shortServerId, byte[] clientCertificate, byte[] clientPrivateKey,
        ...
        return new Security(serverUri, false, SecurityMode.X509.code, clientCertificate.clone(),
                serverPublicKey.clone(), clientPrivateKey.clone(), shortServerId, CertificateUsage.DOMAIN_ISSUER_CERTIFICATE.code);
    }
     ...
     */
    public static Security x509(String serverUri, int shortServerId, byte[] clientCertificate, byte[] clientPrivateKey,
            byte[] serverPublicKey, int certificateUsage) {
        return new Security(serverUri, false, SecurityMode.X509.code, clientCertificate.clone(),
    ...
    public WriteResponse write(ServerIdentity identity, int resourceId, LwM2mResource value) {
        ...
        case SEC_CERTIFICATE_USAGE: // certificate usage
            ...
            certificateUsage = ((Long) value.getValue()).intValue();
            return WriteResponse.success();
